"""Re-seed vehicle makes and models (fix for empty tables after failed deploy).

Revision ID: 010_reseed_vehicle_data
Revises: 009_seed_vehicle_makes_models
Create Date: 2026-02-09 15:00:00.000000

Migration 009 was marked as applied during a failed deploy that crashed before
data was committed. This migration re-seeds the same data unconditionally.
"""
...
revision: str = "010_reseed_vehicle_data"
down_revision: str = "009_seed_vehicle_makes_models"
...
def upgrade() -> None:
    ...
    # Force re-seed makes using ON CONFLICT (handles both empty and partial states)
    for make in MAKES:
        conn.execute(
            sa.text(
...
def downgrade() -> None:
    # No-op: don't delete data on downgrade
    pass
