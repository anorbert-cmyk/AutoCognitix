...
@pytest.mark.asyncio
class TestSeedDtcCodesRejectsUnservableCodes:
    """The seed loader is a write path, and it bypasses `DTCCreate`.

    `_seed_dtc_codes` inserts with raw SQL, so the Pydantic validator that guards
    `POST /api/v1/dtc/` never runs. Migration 021 purges the five malformed rows
    that shipped historically - but a migration runs exactly once. On a database
    that is still EMPTY when it runs (a new Railway environment, a staging
    rebuild, a restore) it purges nothing, stamps itself applied forever, and then
    this function re-inserts them from the seed file.

    The shipped file is clean (asserted in test_dtc_codes.py), but that file is
    regenerated by scripts. This is the runtime backstop for the day it is
    regenerated from a corpus that still has junk in it.
    """

    @staticmethod
    def _session_with_empty_table():
        mock_result = MagicMock()
        mock_result.scalar.return_value = 0
        mock_session = AsyncMock()
        mock_session.execute.return_value = mock_result
    ...
    @staticmethod
    def _seeded_codes(mock_session):
        """Every `code` bound to an INSERT (the SELECT COUNT call has no params)."""
        return [
            call.args[1]["code"]
    ...
    async def test_codes_the_shared_rule_rejects_are_never_inserted(self):
        from app.main import _seed_dtc_codes

        payload = json.dumps(
            {
                "codes": [
                    {"code": "P0300", "description_en": "real code", "category": "powertrain"},
                    {
                        "code": "PEACE",
                        "description_en": "hex-shaped word",
                        "category": "powertrain",
                    },
                    {"code": "UA80E", "description_en": "mfr designation", "category": "network"},
                    {"code": "P0301", "description_en": "real code", "category": "powertrain"},
                ]
            }
        )
        mock_session = self._session_with_empty_table()

        with (
            patch("app.main.async_session_maker", return_value=mock_session),
            patch("pathlib.Path.exists", return_value=True),
            patch("pathlib.Path.open", mock_open(read_data=payload)),
        ):
            await _seed_dtc_codes()

        seeded = self._seeded_codes(mock_session)
        assert seeded == ["P0300", "P0301"], seeded
        assert "PEACE" not in seeded and "UA80E" not in seeded
        mock_session.commit.assert_awaited()

    async def test_codes_are_stored_canonicalised(self):
        """Lowercase in the file must not become a row the detail endpoint misses."""
        from app.main import _seed_dtc_codes

        payload = json.dumps(
            {"codes": [{"code": " p0300 ", "description_en": "x", "category": "powertrain"}]}
        )
        mock_session = self._session_with_empty_table()
        ...
        assert self._seeded_codes(mock_session) == ["P0300"]

    async def test_an_all_junk_seed_file_inserts_nothing(self):
        from app.main import _seed_dtc_codes

        payload = json.dumps(
            {"codes": [{"code": "PEACE", "description_en": "x", "category": "powertrain"}]}
        )
        mock_session = self._session_with_empty_table()
        ...
        assert self._seeded_codes(mock_session) == []
        mock_session.commit.assert_not_awaited()
